...
def test_dimensions():
    ...
    try:
        composition_gan = CompositionGAN(num_components=3, output_dim=feature_dim)
        
        # Test forward pass
        component_features = [torch.randn(batch_size, feature_dim) for _ in range(3)]
    ...
    print("\n4. Testing image feature extraction...")
    try:
        images = torch.randn(batch_size, 3, image_size, image_size)
        flattened = images.view(batch_size, -1)
        print(f"✓ Image flattening: {images.shape} -> {flattened.shape}")
        
        # Test adaptive pooling to match feature dimension
        if flattened.size(1) != feature_dim:
            adapted = torch.nn.functional.adaptive_avg_pool1d(
                flattened.unsqueeze(1), feature_dim
            ).squeeze(1)
            print(f"✓ Adaptive pooling: {flattened.shape} -> {adapted.shape}")
