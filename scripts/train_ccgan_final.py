    ...
    def train_component_gan_epoch(self, dataloader, epoch):
        ...
        for batch_idx, (real_images, text_descriptions) in enumerate(pbar):
            try:
                ...
                # Flatten images to features and ensure 2D
                ...
                image_features = self.ensure_2d(image_features)
                ...
                fake_features = self.ensure_2d(fake_features)
                ...
                if g_loss.item() < 0.5:
                    for i, desc in enumerate(text_descriptions):
                        # Ensure we're storing 1D tensors in cache
                        feature_to_cache = fake_features[i].detach().cpu()
                        if feature_to_cache.dim() > 1:
                            feature_to_cache = feature_to_cache.squeeze(0)
                        self.cache.store_component(desc, feature_to_cache)
    ...
    def train_composition_gan_epoch(self, dataloader, epoch):
        ...
        for batch_idx, (scene_descriptions, component_lists, target_images) in enumerate(pbar):
            try:
                ...
                # Flatten target images and ensure 2D
                ...
                target_features = self.ensure_2d(target_features)
                
                # Train Composition GAN
                self.optimizer_g.zero_grad()
                
                # Retrieve or generate component features
                component_features_batch = []
                    ...
                    for comp_desc in components:
                        if comp_desc:  # Skip empty strings
                            ...
                            if cached_feature is not None:
                                # Ensure cached feature is on device and proper dimension
                                cached_feature = cached_feature.to(self.device)
                                if cached_feature.dim() == 1:
                                    cached_feature = cached_feature.unsqueeze(0)
                                scene_components.append(cached_feature)
                            else:
                                # Generate component if not in cache
                                new_feature = self.component_gan.generate_component(
                                    comp_desc, self.cache, num_samples=1
                                ).to(self.device)
                                new_feature = self.ensure_2d(new_feature)
                                scene_components.append(new_feature)
                    
                    # Pad to required number of components
                    while len(scene_components) < self.config['model']['num_components']:
                        scene_components.append(torch.zeros(1, self.feature_dim).to(self.device))
                    
                    # Stack components for this scene
                    scene_component_tensor = torch.cat(scene_components, dim=0)
                    component_features_batch.append(scene_component_tensor)
                
                # Generate composition for each scene
                generated_compositions = []
                for i in range(batch_size):
                    # Get components for this scene
                    scene_components = [component_features_batch[i][j] for j in range(self.config['model']['num_components'])]
                    composition = self.composition_gan(scene_components, scene_embeddings[i])
                    composition = self.ensure_2d(composition)
                    generated_compositions.append(composition)
                
                # Stack all compositions
                generated_tensor = torch.cat(generated_compositions, dim=0)
    ...
    def train_cpp_epoch(self, dataloader, epoch):
        ...
        for batch_idx, (images, preference_scores) in enumerate(pbar):
            try:
                ...
                # Flatten images for CPP and ensure 2D
...
def main():
    ...
    # Initialize trainer
    trainer = CCGANFinalTrainer(config)
